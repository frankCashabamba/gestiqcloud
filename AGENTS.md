...
## M3 — E‑invoicing (SRI/SII) — Agente 4
**Objetivo**: colas y tareas de envío/consulta.

**Tareas**
- Migración `100_einvoicing_queues.sql`: `sri_submissions`, `sii_batches`, `sii_batch_items` + enums.
- Celery: `sign_and_send(invoice_id)` (SRI, XAdES‑BES, Recepción/Autorización), `build_and_send_sii(period)`.
- Endpoints `/einvoicing`: send/consult; “explicar error”.
- Credenciales por tenant (secure storage).

**Aceptación**
- EC: una factura **AUTHORIZED** (staging). ES: un lote **ACCEPTED**.
