...
def gc_refresh_tokens(db: Session, older_than_days: int = 60) -> int:
    ...
    n = 0
    n += db.query(RefreshToken).filter(RefreshToken.expires_at < cutoff).delete(synchronize_session=False)
    n += db.query(RefreshTokenFamily).filter(~RefreshTokenFamily.tokens.any()).delete(synchronize_session=False)
    db.commit()
    return n
