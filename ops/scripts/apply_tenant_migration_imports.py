...
MIGRATION_DIR = project_root / "ops" / "migrations" / "2025-10-17_050_add_tenant_id_to_imports"
...
def validate_prerequisites(engine):
    ...
    with engine.connect() as conn:
        ...
        if not result.scalar():
            print("ERROR: public.tenants table does not exist. Run tenant bootstrap migration first.")
            return False
        ...
        tables = ['import_batches', 'import_items', 'import_mappings', 
        ...
        for table in tables:
            ...
            """))
            if not result.scalar():
                print(f"WARNING: Table {table} does not exist. Migration may be incomplete.")
...
def main():
    ...
    parser.add_argument(
        "--rollback",
        action="store_true",
        help="Rollback migration (apply down.sql)"
    )
