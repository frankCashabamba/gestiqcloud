...
--
-- Estrategia más robusta: usa DELETE ... USING con un JOIN explícito
-- en vez de NOT IN (subquery), evitando edge cases con grandes conjuntos.
-- Es idempotente: si el índice único ya existe, no hace nada.
...
BEGIN
    ...
    ELSE
        ...
        IF to_regclass('public.imp_documento_successor') IS NOT NULL THEN
            -- Normalizar vínculos: remapear predecessor/successor al superviviente
        ...
        -- ----------------------------------------------------------------
        -- 4. Eliminar duplicados usando JOIN explícito (más robusto que NOT IN)
        -- ----------------------------------------------------------------
