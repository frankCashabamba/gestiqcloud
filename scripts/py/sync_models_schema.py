...
def _ensure_env_dsn(dsn: str | None) -> str:
    ...
    if not resolved:
        raise SystemExit(
            "Provide --dsn or set DB_DSN / DATABASE_URL before running sync_models_schema.py"
        )
...
def main() -> int:
    ...
    try:
        ...
        raise SystemExit(
            f"[sync_models_schema] Failed to import models: {exc}"
        ) from exc
    ...
    try:
        ...
        print(
            f"[sync_models_schema] Warning: could not ensure pgcrypto extension ({exc})"
        )
    ...
    try:
        ...
        raise SystemExit(
            f"[sync_models_schema] Failed to create tables: {exc}"
        ) from exc

    print(
        f"[sync_models_schema] Ensured {len(Base.metadata.tables)} tables are present."
    )
    return 0
