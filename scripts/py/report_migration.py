...
EXCLUDE_DIRS = {".git", ".hg", ".svn", ".mypy_cache", ".pytest_cache", ".venv", "venv",
                "__pycache__", "node_modules", "dist", "build"}

def _python_grep(pattern: str, dirpath: Path, globs: tuple[str, ...] = ("*.py",)) -> list[str]:
...
def report_duplicates() -> str:
    try:
        out = subprocess.check_output(
            ["python", str(ROOT / "scripts" / "py" / "scan_duplicates.py")],
            text=True
        )
        return out
