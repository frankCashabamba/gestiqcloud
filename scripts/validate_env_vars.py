...
def validate_environment(env_type: str = "production") -> bool:
    ...
    if all_valid:
        print(
            f"{Colors.GREEN}✅ Todas las variables críticas están bien configuradas{Colors.END}\n"
        )
        return True
    else:
        print(
            f"{Colors.RED}❌ Hay variables críticas no configuradas o mal configuradas{Colors.END}\n"
        )
        return False
...
def print_checklist():
    ...
    checklist_items = [
        (
            "DEFAULT_FROM_EMAIL",
            "no es localhost",
            os.getenv("DEFAULT_FROM_EMAIL", "").count("localhost") == 0,
        ),
        (
            "REDIS_URL",
            "no es localhost",
            os.getenv("REDIS_URL", "").count("localhost") == 0,
        ),
        (
            "CORS_ORIGINS",
            "no contiene localhost",
            os.getenv("CORS_ORIGINS", "").count("localhost") == 0,
        ),
        (
            "DATABASE_URL",
            "no es localhost",
            os.getenv("DATABASE_URL", "").count("localhost") == 0,
        ),
        ...
        (
            "CERT_PASSWORD",
            "no es placeholder",
            "TODO" not in os.getenv("CERT_PASSWORD", ""),
        ),
    ]
