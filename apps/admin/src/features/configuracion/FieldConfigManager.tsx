...
export default function FieldConfigManager() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      if (empresa) {
        const { data } = await api.get(`/api/v1/company/settings/fields`, { params: { module: moduleKey, empresa } })
        setItems(data.items || [])
      } else {
        const { data } = await api.get(`/api/v1/admin/field-config/sector`, { params: { module: moduleKey, sector } })
        setItems(data.items || [])
      }
    } catch (e: any) {
  ...
  async function save() {
    ...
    try {
      if (empresa) {
        await api.put(`/api/v1/admin/field-config/tenant`, { empresa, module: moduleKey, items })
        await api.put(`/api/v1/admin/field-config/tenant/mode`, { empresa, module: moduleKey, form_mode: formMode })
      } else {
        await api.put(`/api/v1/admin/field-config/sector`, { sector, module: moduleKey, items })
      }
