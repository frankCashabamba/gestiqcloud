...
import { listCategoriasGasto, removeCategoriaGasto, type CategoriaGasto } from '../../../services/configuracion/categorias-gasto'
...
export default function CategoriaGastoList() {
  ...
  useEffect(() => {
    (async () => {
      try {
        setLoading(true)
        setItems(await listCategoriasGasto())
      } catch (e: any) {
  ...
  return (
    <div style={{ padding: 16 }}>
      <div className="flex justify-between items-center mb-3">
        <h3 className="text-xl font-semibold">Categorías de gasto</h3>
      ...
      <table className="min-w-full bg-white border border-gray-200 rounded">
        <thead className="bg-gray-50">
          <tr>
            ...
            <th className="text-left py-2 px-3">Categoría padre</th>
        ...
        <tbody>
          {items.map((it) => (
            <tr key={it.id} className="border-t">
              ...
              <td className="py-2 px-3">{it.parent_code}</td>
              ...
              <td className="py-2 px-3">
                ...
                <button className="text-red-700" onClick={async () => {
                  ...
                  try {
                    await removeCategoriaGasto(it.id)
          ...
          {!loading && items.length === 0 && (
            <tr><td className="py-3 px-3" colSpan={5}>Sin categorías de gasto</td></tr>
          )}
        </tbody>
      </table>
    </div>
