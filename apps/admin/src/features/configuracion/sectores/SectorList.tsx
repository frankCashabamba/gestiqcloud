...
import { listSectores, removeSector, type Sector } from '../../../services/configuracion/sectores'
        ...
        <tbody>
          {items.map((it) => (
            <tr key={it.id} className="border-t">
              ...
              <td className="py-2 px-3">{it.tipo_empresa_id}</td>
              <td className="py-2 px-3">{it.tipo_negocio_id}</td>
          ...
          {!loading && items.length === 0 && (
            <tr><td className="py-3 px-3" colSpan={4}>Sin sectores</td></tr>
          )}
        </tbody>
      </table>
    </div>
