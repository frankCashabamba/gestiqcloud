...
class TestESValidator:
    ...
    def test_validate_nif_valid(self):
        errors = self.validator.validate_tax_id("12345678Z")
        assert len(errors) == 0

    def test_validate_nif_invalid_letter(self):
        errors = self.validator.validate_tax_id("12345678X")
        assert len(errors) == 1
        assert errors[0]["code"] == "INVALID_TAX_ID_CHECKSUM"

    def test_validate_nie_valid(self):
        errors = self.validator.validate_tax_id("X1234567L")
        assert len(errors) == 0

    def test_validate_nie_invalid_letter(self):
        errors = self.validator.validate_tax_id("X1234567Z")
        assert len(errors) == 1
        assert errors[0]["code"] == "INVALID_TAX_ID_CHECKSUM"

    def test_validate_cif_valid_number(self):
        errors = self.validator.validate_tax_id("A12345674")
        assert len(errors) == 0
    ...
    def test_validate_cif_invalid_control(self):
        errors = self.validator.validate_tax_id("A12345679")
        assert len(errors) == 1
        assert errors[0]["code"] == "INVALID_TAX_ID_CHECKSUM"

    def test_validate_tax_id_invalid_format(self):
        errors = self.validator.validate_tax_id("123ABC")
        assert len(errors) == 1
    ...
    def test_validate_tax_rates_valid(self):
        errors = self.validator.validate_tax_rates([0.0, 4.0, 10.0, 21.0])
        assert len(errors) == 0

    def test_validate_tax_rates_invalid(self):
        errors = self.validator.validate_tax_rates([5.0, 12.0, 16.0])
        assert len(errors) == 3
    ...
    def test_validate_invoice_number_valid_formats(self):
        ...
        for number in valid_numbers:
            errors = self.validator.validate_invoice_number(number)
            assert len(errors) == 0, f"Failed for: {number}"

    def test_validate_invoice_number_too_long(self):
        errors = self.validator.validate_invoice_number("A" * 31)
        assert len(errors) == 1
