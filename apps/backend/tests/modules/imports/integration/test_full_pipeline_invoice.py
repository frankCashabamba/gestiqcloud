...
def test_tenant_ec(db_session: Session) -> dict:
    ...
    db_session.commit()
    return {"tenant_id": tenant_id, "country": "EC"}
...
def test_full_pipeline_invoice_ec(
    db_session: Session, test_tenant_ec: dict, factura_ec_sample: Path
):
    """
...
def test_pipeline_duplicate_detection(
    ...
    batch = create_batch(db_session, tenant_id, "invoices", "Test duplicates")
...
def test_pipeline_validation_errors(db_session: Session, test_tenant_ec: dict):
    ...
    batch = create_batch(db_session, tenant_id, "invoices", "Test validación")
