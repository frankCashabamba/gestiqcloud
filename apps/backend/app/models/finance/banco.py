...
class BancoMovimiento(Base):
    ...
    saldo_anterior: Mapped[float] = mapped_column(Numeric(12, 2), nullable=False, default=0)
    saldo_nuevo: Mapped[float] = mapped_column(Numeric(12, 2), nullable=False)
    ref_bancaria: Mapped[str | None] = mapped_column(String(100), nullable=True)
    conciliado: Mapped[bool] = mapped_column(Boolean, nullable=False, default=False, index=True)
