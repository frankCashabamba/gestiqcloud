...
class Payment(Base):
    ...
    scheduled_date: Mapped[date | None] = mapped_column(
        Date, nullable=True,
        comment="Fecha programada (si está programado)"
    )
    ...
    method: Mapped[str] = mapped_column(
        payment_method, nullable=False, default="BANK_TRANSFER",
        comment="CASH, BANK_TRANSFER, CARD, CHEQUE, DIRECT_DEBIT, OTHER"
    )
    status: Mapped[str] = mapped_column(
        payment_status, nullable=False, default="PENDING",
    ...
    ref_doc_type: Mapped[str] = mapped_column(
        ...
        comment="Tipo de documento (invoice, bill, payroll, etc)"
    )
    ...
    last_error: Mapped[str | None] = mapped_column(
        ...
        comment="Último error ocurrido"
    )
    
    # Bank reference
    bank_reference: Mapped[str | None] = mapped_column(
        String(100), nullable=True,
        comment="Referencia del banco (número de transacción)"
    )
...
class PaymentSchedule(Base):
    ...
    end_date: Mapped[date | None] = mapped_column(
        Date, nullable=True,
        comment="Fecha de fin (calculada)"
    )
