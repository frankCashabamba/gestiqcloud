...
class EInvoice(Base):
    ...
    country: Mapped[str] = mapped_column(
        ...
        comment="País destino (ES, EC, MX, CL, CO)"
    )
    ...
    authorization_code: Mapped[str | None] = mapped_column(
        ...
        comment="Código de autorización"
    )
    ...
    status: Mapped[str] = mapped_column(
        einvoice_status, nullable=False, default="PENDING",
    ...
    next_retry_at: Mapped[datetime | None] = mapped_column(
        TIMESTAMP(timezone=True), nullable=True,
        comment="Próxima fecha de reintento"
    )
    
    # Notes and errors
    notes: Mapped[str | None] = mapped_column(
        Text, nullable=True,
        comment="Notas sobre el envío"
    )
    ...
    errors: Mapped[list["EInvoiceError"]] = relationship(
        "EInvoiceError", back_populates="einvoice",
        cascade="all, delete-orphan", lazy="selectin"
    )
    signature: Mapped["EInvoiceSignature | None"] = relationship(
        "EInvoiceSignature", back_populates="einvoice",
...
class EInvoiceSignature(Base):
    ...
    certificate_serial: Mapped[str | None] = mapped_column(
        ...
        comment="Número de serie del certificado"
    )
    certificate_issuer: Mapped[str | None] = mapped_column(
        String(255), nullable=True,
        comment="Emisor del certificado"
    )
    certificate_subject: Mapped[str | None] = mapped_column(
        String(255), nullable=True,
        comment="Sujeto del certificado"
    )
    ...
    signature_value: Mapped[str | None] = mapped_column(
        Text, nullable=True,
        comment="Valor de la firma (base64)"
    )
    digest_value: Mapped[str | None] = mapped_column(
        String(255), nullable=True,
        comment="Valor del digest"
    )
    digest_algorithm: Mapped[str] = mapped_column(
        String(50), nullable=False, default="SHA256",
        comment="Algoritmo de hash (SHA256, SHA1, etc)"
    )
    
    # Status
    status: Mapped[str] = mapped_column(
        signature_status, nullable=False, default="NOT_SIGNED",
...
class EInvoiceStatus(Base):
    ...
    changed_by: Mapped[uuid.UUID | None] = mapped_column(
        PGUUID(as_uuid=True), nullable=True,
        comment="Usuario que hizo el cambio"
    )
...
class EInvoiceError(Base):
    ...
    error_detail: Mapped[str | None] = mapped_column(
        Text, nullable=True,
        comment="Detalles técnicos"
    )
    error_type: Mapped[str] = mapped_column(
        String(50), nullable=False,
        comment="VALIDATION, CONNECTIVITY, SIGNATURE, SERVER, OTHER"
    )
    
    # Recovery
    recovery_action: Mapped[str | None] = mapped_column(
        String(500), nullable=True,
        comment="Acción recomendada para resolver"
    )
    is_recoverable: Mapped[bool] = mapped_column(
        Boolean, nullable=False, default=True,
        comment="Si se puede recuperar con reintento"
    )
    ...
    # Relations
    einvoice: Mapped["EInvoice"] = relationship(
        "EInvoice", back_populates="errors", lazy="select"
    )
