...
class EInvoicingCountrySettings(Base):
    ...
    api_key_encrypted: Mapped[str | None] = mapped_column(
        String(500), nullable=True,
        comment="API key cifrada (si aplica)"
    )
    
    # Validation rules (JSON)
    validation_rules: Mapped[dict | None] = mapped_column(
        JSON, nullable=True,
        comment="Reglas de validación específicas del país (JSON)"
    )
    
    # Retry policy
    max_retries: Mapped[int] = mapped_column(
        nullable=False, default=5,
        comment="Máximo número de reintentos"
    )
    retry_backoff_seconds: Mapped[int] = mapped_column(
        nullable=False, default=300,
        comment="Segundos iniciales para exponential backoff"
    )
