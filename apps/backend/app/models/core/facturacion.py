...
class Invoice(Base):
    ...
    cliente_id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), ForeignKey("clients.id"))
    ...
    lineas: Mapped[list[LineaFactura]] = relationship(LineaFactura, cascade="all, delete-orphan")
...
class BankTransaction(Base):
    ...
    referencia: Mapped[str | None] = mapped_column(nullable=True)
    contrapartida_nombre: Mapped[str | None] = mapped_column(nullable=True)
    contrapartida_iban: Mapped[str | None] = mapped_column(nullable=True)
    banco_contraparte: Mapped[str | None] = mapped_column(nullable=True)

    comision: Mapped[float] = mapped_column(default=0)
    fuente: Mapped[str] = mapped_column(default="ocr")
    adjunto_url: Mapped[str | None] = mapped_column(nullable=True)

    sepa_end_to_end_id: Mapped[str | None] = mapped_column(nullable=True)
    sepa_creditor_id: Mapped[str | None] = mapped_column(nullable=True)
    sepa_mandate_ref: Mapped[str | None] = mapped_column(nullable=True)
    esquema_sepa: Mapped[str | None] = mapped_column(nullable=True)
