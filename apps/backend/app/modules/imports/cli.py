...
def import_folder(
    ...
    doc_type: Optional[str] = typer.Option(None, "--doc-type", "-t", help="Tipo de documento (invoice, expense, product, bank_tx)"),
    parser_id: Optional[str] = typer.Option(None, "--parser", help="ID del parser específico"),
    ...
    try:
        ...
        if not dry_run and canonical_docs:
            output_file = folder / f"canonical_export_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
...
def validate_file(
    ...
    country: Optional[str] = typer.Option(None, "--country", "-c", help="Código país (EC, ES, etc.)")
):
...
def health():
    ...
    try:
        ...
        try:
            ...
            typer.echo(f"  ⚠️  Database: No connection (optional)")
