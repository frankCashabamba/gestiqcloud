...
class BenchmarkResult:
    ...
    regression: Optional[float] = None  # percentage point change
...
class BenchmarkReport:
    ...
    def summary(self) -> str:
        ...
        for result in self.results:
            ...
            lines.append(f"{icon} {result.metric_name}: {result.current_value:.1f}% (threshold: {result.threshold:.1f}%){regression_str}")
    ...
    def get_deployment_decision(self, report: BenchmarkReport) -> dict:
        ...
        if failed_metrics:
            ...
            for r in failed_metrics:
                reason_lines.append(f"  - {r.metric_name}: {r.current_value:.1f}% (threshold: {r.threshold:.1f}%)")
