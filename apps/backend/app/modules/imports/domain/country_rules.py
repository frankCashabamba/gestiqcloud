...
class CountryRuleSet(ABC):
    ...
    @abstractmethod
    def validate_invoice_number(self, invoice_number: str) -> tuple[bool, Optional[str]]:
        """Validate invoice number format."""
        pass
    
    @abstractmethod
    def validate_fiscal_date(self, date: str) -> tuple[bool, Optional[str]]:
...
class PeruRuleSet(CountryRuleSet):
    ...
    def validate_invoice_number(self, invoice_number: str) -> tuple[bool, Optional[str]]:
        ...
        if re.match(PeruRules.INVOICE_SERIES_FORMAT, str(invoice_number)):
            return True, None
        return False, f"Peru: Invoice format should be like F-001, FA-1000"
...
class ColombiaRuleSet(CountryRuleSet):
    ...
    def validate_invoice_number(self, invoice_number: str) -> tuple[bool, Optional[str]]:
        ...
        if re.match(ColombiaRules.INVOICE_SERIES_FORMAT, str(invoice_number)):
            return True, None
        return False, f"Colombia: Invoice format should be like F-001, FA-1000"
...
class CountryRulesRegistry:
    ...
    def get_rules(self, country: Country) -> Optional[CountryRuleSet]:
