...
def test_preprocess_validates_file_size():
    ...
    with patch("app.modules.imports.application.tasks.task_preprocess.session_scope") as mock_session:
        mock_db = MagicMock()
...
def test_classify_document_returns_factura():
    ocr_result = {
        "documentos": [
            {"texto": "FACTURA NIF B12345678 IVA 21%"}
        ]
    }
    
    result = _classify_document(ocr_result)
...
def test_classify_document_banco():
    ocr_result = {
        "documentos": [
            {"texto": "EXTRACTO BANCARIO IBAN ES12 3456 SALDO 1000"}
        ]
    }
