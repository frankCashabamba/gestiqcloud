...
def _extract_invoice_data(root: ET.Element, namespaces: Dict) -> Optional[Dict[str, Any]]:
...
def _extract_ubl_invoice(root: ET.Element, ns: str) -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        if legal_total is not None:
            totals = {
                ...
                "tax": _to_float(_get_text(legal_total, f".//{tag('TaxTotal')}/{tag('TaxAmount')}")),
        ...
        for line_elem in root.findall(f".//{tag('InvoiceLine')}"):
            line = {
                ...
                "unit_price": _to_float(_get_text(line_elem, f".//{tag('Price')}/{tag('PriceAmount')}")),
...
def _extract_cfdi_invoice(root: ET.Element, ns: str) -> Optional[Dict[str, Any]]:
...
def _extract_generic_invoice(root: ET.Element) -> Optional[Dict[str, Any]]:
...
def _get_text(elem: ET.Element, xpath: str) -> Optional[str]:
