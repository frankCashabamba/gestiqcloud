...
def _map_columns(header: tuple) -> Dict[str, int]:
    ...
    for idx, col_name in enumerate(header):
        ...
        # Description
        elif any(kw in col_lower for kw in ["description", "descripcion", "desc", "concepto", "detail"]):
...
def _parse_expense_row(row: tuple, row_idx: int, col_map: Dict[str, int]) -> Optional[Dict[str, Any]]:
    ...
    description = row[col_map["description"]] if "description" in col_map and col_map["description"] < len(row) else None
    category = row[col_map["category"]] if "category" in col_map and col_map["category"] < len(row) else None
    amount = row[col_map["amount"]] if "amount" in col_map and col_map["amount"] < len(row) else None
    vendor = row[col_map["vendor"]] if "vendor" in col_map and col_map["vendor"] < len(row) else None
    payment_method = row[col_map["payment_method"]] if "payment_method" in col_map and col_map["payment_method"] < len(row) else None
    reference = row[col_map["reference"]] if "reference" in col_map and col_map["reference"] < len(row) else None
    tax = row[col_map["tax"]] if "tax" in col_map and col_map["tax"] < len(row) else None
    currency = row[col_map["currency"]] if "currency" in col_map and col_map["currency"] < len(row) else None
