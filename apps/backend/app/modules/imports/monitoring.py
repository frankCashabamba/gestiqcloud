...
class OCRMetricsCollector:
    ...
    def record(self, job_id: str, duration_ms: float, status: str) -> None:
        ...
        with self._lock:
            self._metrics.append(ProcessingMetric(
                job_id=job_id,
    ...
    def get_processing_times(self) -> dict[str, float | None]:
        ...
        return {
            ...
            "p95_ms": round(durations_sorted[int(n * 0.95)] if n >= 20 else durations_sorted[-1], 2),
            "p99_ms": round(durations_sorted[int(n * 0.99)] if n >= 100 else durations_sorted[-1], 2),
        }
...
def get_queue_metrics(db: Session | None = None) -> dict[str, Any]:
    ...
    def _query(session: Session) -> dict[str, Any]:
        ...
        ).group_by(ImportOCRJob.status).all()
        
        status_map = {row.status: row.count for row in status_counts}
        
        # Oldest pending job age
        oldest_pending = session.query(
            func.min(ImportOCRJob.created_at)
        ).filter(ImportOCRJob.status == "pending").scalar()
...
def get_all_metrics(db: Session | None = None) -> dict[str, Any]:
    ...
    return {
        ...
        "health": {
            ...
            "backlog_age_healthy": (
                queue["oldest_pending_age_seconds"] is None or 
                queue["oldest_pending_age_seconds"] < 300
            ),
        },
    }
