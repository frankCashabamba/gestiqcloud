...
class InvoiceFromReceiptRequest(BaseModel):
    ...
    customer_id: str = Field(
        ...,
        description="ID del cliente con datos fiscales completos"
    )
...
@router.post(
    "/{receipt_id}/invoice",
    response_model=InvoiceFromReceiptResponse,
    status_code=201
)
def create_invoice_from_pos_receipt(
    ...
    if not customer:
        raise HTTPException(
            status_code=404,
            detail=f"Cliente {payload.customer_id} no encontrado"
        )
    ...
    try:
        ...
        invoice_number = db.execute(
            text("SELECT numero FROM invoices WHERE id = :id"),
            {"id": invoice_id}
        ).scalar()
        
        receipt_number = db.execute(
            text("SELECT number FROM pos_receipts WHERE id = :id"),
            {"id": receipt_uuid}
        ).scalar()
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Error al crear factura desde recibo: {str(e)}"
        )


@router.get("/{receipt_id}/invoice")
def get_invoice_from_receipt(
    receipt_id: str,
    request: Request,
    db: Session = Depends(get_db)
):
    ...
    if not result:
        raise HTTPException(
            status_code=404,
            detail=f"Recibo {receipt_id} no encontrado"
        )
    
    if not result[2]:  # invoice_id
        raise HTTPException(
            status_code=404,
            detail=f"El recibo {result[1]} no tiene factura asociada"
        )
...
def unlink_invoice_from_receipt(
    receipt_id: str,
    request: Request,
    db: Session = Depends(get_db)
):
    ...
    if not result:
        raise HTTPException(
            status_code=404,
            detail="Recibo no encontrado o sin factura vinculada"
        )
