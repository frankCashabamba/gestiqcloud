...
class ActualizarCliente(BaseUseCase[ClienteRepo]):
    def execute(self, *, id: Any, tenant_id: Any, data: ClienteIn) -> ClienteOut:
        ...
        return ClienteOut(
            ...
            identificacion_tipo=updated.identificacion_tipo,
            email=updated.email,
...
class ListarClientes(BaseUseCase[ClienteRepo]):
    def execute(self, *, tenant_id: Any, limit: int = 200, offset: int = 0, search: str | None = None) -> Sequence[ClienteOut]:
        items = self.repo.list(tenant_id=tenant_id, limit=limit, offset=offset, search=search)
        return [
            ClienteOut(
                id=i.id or 0,
                nombre=i.nombre,
                identificacion=i.identificacion,
                identificacion_tipo=i.identificacion_tipo,
                email=i.email,
                telefono=i.telefono,
