...
from app.modules.crm.domain.models import Lead, Opportunity, Activity
from app.modules.crm.domain.entities import LeadStatus, OpportunityStage, ActivityStatus
...
class CRMService:
    ...
    def get_lead(self, tenant_id: UUID, lead_id: UUID) -> Optional[LeadOut]:
    ...
    def update_lead(self, tenant_id: UUID, lead_id: UUID, data: LeadUpdate) -> Optional[LeadOut]:
        lead = self.db.query(Lead).filter(
            Lead.id == lead_id,
            Lead.tenant_id == tenant_id
        ).first()
    ...
    def get_opportunity(self, tenant_id: UUID, opp_id: UUID) -> Optional[OpportunityOut]:
    ...
    def get_dashboard(self, tenant_id: UUID) -> DashboardMetrics:
        ...
        total_opportunities = self.db.query(func.count(Opportunity.id)).filter(
            Opportunity.tenant_id == tenant_id
        ).scalar() or 0
        
        total_pipeline_value = self.db.query(
        ...
        won_opportunities = self.db.query(func.count(Opportunity.id)).filter(
            Opportunity.tenant_id == tenant_id,
            Opportunity.stage == OpportunityStage.CLOSED_WON
        ).scalar() or 0
        
        lost_opportunities = self.db.query(func.count(Opportunity.id)).filter(
            Opportunity.tenant_id == tenant_id,
        ...
        for status in LeadStatus:
            count = self.db.query(func.count(Lead.id)).filter(
                Lead.tenant_id == tenant_id,
                Lead.status == status
            ).scalar() or 0
