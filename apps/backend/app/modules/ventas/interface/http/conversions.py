...
@router.post(
    "/{order_id}/invoice",
    response_model=InvoiceFromOrderResponse,
    status_code=201
)
def create_invoice_from_sales_order(
    order_id: int,
    request: Request,
    payload: Optional[InvoiceFromOrderRequest] = None,
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Error al crear factura desde orden: {str(e)}"
        )


@router.get("/{order_id}/invoice")
def get_invoice_from_order(
    order_id: int,
    request: Request,
    db: Session = Depends(get_db)
):
    ...
    if not result:
        raise HTTPException(
            status_code=404,
            detail=f"La orden {order_id} no tiene factura asociada"
        )
