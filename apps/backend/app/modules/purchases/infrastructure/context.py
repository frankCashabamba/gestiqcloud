...
def get_context_summary(db: Session, tenant_id: str) -> dict[str, Any]:
    ...
    pending = db.execute(
        text(
            ...
            "FROM purchases WHERE tenant_id = :tid AND status IN ('draft', 'sent', 'confirmed')"
        ),
        {"tid": tenant_id},
    ).fetchone()
    ...
    recent = db.execute(
        text(
            "SELECT po.id, s.name AS supplier_name, po.total AS total_amount, po.status, po.created_at "
            "FROM purchases po "
