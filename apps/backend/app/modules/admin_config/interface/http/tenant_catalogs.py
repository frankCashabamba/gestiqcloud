...
def list_units(db: Session = Depends(get_db)):
    rows = db.query(UnitOfMeasure).filter(UnitOfMeasure.active.is_(True)).order_by(UnitOfMeasure.code).all()
    return [{"id": r.id, "code": r.code, "name": r.name, "abbreviation": r.abbreviation} for r in rows]


@router.get("/doc-types")
def list_doc_types(db: Session = Depends(get_db)):
    rows = db.query(DocumentType).filter(DocumentType.active.is_(True)).order_by(DocumentType.code).all()
    return [{"id": r.id, "code": r.code, "name": r.name, "description": r.description} for r in rows]
...
def list_expense_categories(db: Session = Depends(get_db)):
    ...
    return [{"id": r.id, "code": r.code, "name": r.name, "parent_code": r.parent_code} for r in rows]
