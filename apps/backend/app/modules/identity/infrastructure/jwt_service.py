...
class JwtService:
    def __init__(self, cfg: JwtSettings | None = None):
        ...
        import logging
        logger = logging.getLogger(__name__)
        logger.debug(f"JwtService created with secret_len={len(self.cfg.secret)}, algorithm={self.cfg.algorithm}")
    ...
    def decode(self, token: str, *, expected_kind: str | None = None) -> Mapping[str, Any]:
        ...
        try:
            ...
            logger.error(f"JWT decode failed: {type(e).__name__}: {e}, secret_len={len(self.cfg.secret)}, secret_first_10={self.cfg.secret[:10]}, secret_hash={hash(self.cfg.secret)}")
            raise
        ...
        if expected_kind is not None:
            ...
            if tok_kind != expected_kind:
                logger.error(f"Token kind mismatch: expected={expected_kind}, got={tok_kind}, payload_keys={list(payload.keys())}")
