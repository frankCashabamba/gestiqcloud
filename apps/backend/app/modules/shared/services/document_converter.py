...
class DocumentConverter:
    ...
    def sales_order_to_invoice(
        ...
        order = self.db.query(SalesOrder).filter(
            ...
            SalesOrder.tenant_id == str(tenant_id)
        ).first()
        ...
        if existing_invoice:
            raise ValueError(
                f"La orden {sales_order_id} ya tiene factura: {existing_invoice[0]}"
            )
    ...
    def pos_receipt_to_invoice(
        ...
        if receipt.status != "paid":
            raise ValueError(
                f"El recibo debe estar pagado (estado actual: {receipt.status})"
            )
        
        if receipt.invoice_id:
            raise ValueError(
                f"El recibo ya tiene factura: {receipt.invoice_id}"
            )
        ...
        lines = self.db.query(POSReceiptLine).filter(
    ...
    def quote_to_sales_order(
        ...
        raise NotImplementedError(
            "Módulo de presupuestos (quotes) no implementado aún"
        )
    ...
    def get_document_chain(
        ...
        raise NotImplementedError(
            "Trazabilidad de documentos pendiente de implementación"
        )
