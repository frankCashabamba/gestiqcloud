...
def load_ocr_runtime_config(db: Any | None = None) -> dict[str, Any]:
    ...
    config: dict[str, Any] = {
        ...
        "ocr_min_quality": float(_DEFAULT_OCR_CONFIG["ocr_min_quality"]),
        "ocr_min_words_for_vision": int(_DEFAULT_OCR_CONFIG["ocr_min_words_for_vision"]),
        ...
        try:
            ...
            for row in rows:
                ...
                if key == "ai_enabled" and row.value_text is not None:
                    ...
                    key in {"pre_extract_image_force_ai", "pipeline_reject_low_quality_enabled"}
                    and row.value_text is not None
                ):
                    config[key] = _bool_value(row.value_text, bool(config.get(key, True)))
                elif (
