...
def ensure_username_unique(
    db: Session, username: str | None, *, exclude_usuario_id: int | None = None
) -> None:
    if not username:
        return
...
def validate_modulos_contratados(db: Session, tenant_id: int, modulos: list[int]) -> None:
    ...
    if faltantes:
        raise HTTPException(
            status_code=400,
            detail=f"Los módulos {faltantes} no están habilitados para la empresa.",
        )
...
def ensure_not_last_admin(db: Session, tenant_id: int, *, usuario_id: int | None = None) -> None:
    ...
    if admins <= 1 and usuario_id is not None:
        ...
        existing = repo.get_usuario_by_id(db, usuario_id, tenant_id)
        ...
        raise HTTPException(
            status_code=400,
            detail="Debe existir al menos un administrador activo en la empresa.",
        )
