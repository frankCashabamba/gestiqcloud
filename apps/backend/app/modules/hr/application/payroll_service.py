...
from app.models.hr.employee import Employee, EmployeeSalary, EmployeeDeduction
...
class PayrollService:
    ...
    @staticmethod
    def calculate_social_security(gross: Decimal, country: str, year: int, employee: bool = True) -> Decimal:
    ...
    def generate_payroll(
        ...
        for employee in employees:
            detail = PayrollService._calculate_employee_payroll(
                db, employee, payroll_month, year
            )
            ...
            payslip = PaymentSlip(
                ...
                valid_until=date(year if payroll_month.endswith("12") else year, 
                                int(payroll_month[5:]) + 1 if not payroll_month.endswith("12") else 1, 1),
                status="GENERATED",
            )
            db.add(payslip)
        ...
        tax_records = [
            ...
            PayrollTax(payroll_id=payroll.id, tax_type="SOCIAL_SECURITY_EMPLOYEE", total_amount=total_ss_employee),
            PayrollTax(payroll_id=payroll.id, tax_type="SOCIAL_SECURITY_EMPLOYER", total_amount=total_ss_employer),
        ]
    ...
    def _calculate_employee_payroll(
        ...
        ss_employee = PayrollService.calculate_social_security(gross, employee.country, year, employee=True)
        mutual = PayrollService.calculate_social_security(gross, employee.country, year, employee=True) * Decimal("0.25")  # Promedio 1.25%
