...
class ConflictResolver:
    ...
    def resolve_stock_conflict(self, local: Dict[str, Any], remote: Dict[str, Any]) -> Dict[str, Any]:
        """
        ...
        if local_time > remote_time:
            return {**remote, **local, 'conflict_resolved': True, 'resolution': 'local_wins'}
        else:
            return {**local, **remote, 'conflict_resolved': True, 'resolution': 'remote_wins'}

    def resolve_receipt_conflict(self, local: Dict[str, Any], remote: Dict[str, Any]) -> Dict[str, Any]:
        """
    ...
    def resolve_product_conflict(self, local: Dict[str, Any], remote: Dict[str, Any]) -> Dict[str, Any]:
        """
        ...
        if local.get('price') != remote.get('price'):
            merged['price'] = max(local.get('price', 0), remote.get('price', 0))  # Take higher price as conservative
    ...
    def resolve_conflict(self, entity_type: str, local: Dict[str, Any], remote: Dict[str, Any]) -> Dict[str, Any]:
        """
...
async def handle_sync_conflicts(conflicts: List[Dict[str, Any]], tenant_id: str, db: Session) -> List[Dict[str, Any]]:
    """
...
def log_conflict_resolution(db: Session, tenant_id: str, original_conflict: Dict[str, Any], resolution: Dict[str, Any]):
    """
