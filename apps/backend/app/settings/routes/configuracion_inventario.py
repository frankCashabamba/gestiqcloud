...
@router.get("/api/configuracion-inventario/{empresa_id}", response_model=schemas.ConfiguracionInventarioOut)
def obtener_config(empresa_id: int, db: Session = Depends(get_db)):
    """ Function obtener_config - auto-generated docstring. """
    config = crud.get_by_empresa(db, empresa_id)
    if not config:
...
@router.put("/api/configuracion-inventario/{empresa_id}", response_model=schemas.ConfiguracionInventarioOut)
def actualizar_config(empresa_id: int, config_in: schemas.ConfiguracionInventarioUpdate, db: Session = Depends(get_db)):
