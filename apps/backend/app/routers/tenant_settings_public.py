...
def get_tenant_config(
    ...
    # Defaults (prioriza template_config si existe, luego settings de tenant)
    ...
    # Defaults tomando primero template_config, luego settings de BD
    currency = (
        settings_row.currency
        if settings_row and settings_row.currency
        else (tenant.base_currency if tenant else None)
    )
    locale = settings_row.locale if settings_row and settings_row.locale else None
    ...
    # Enabled modules: prefer company_modules, fallback to settings.modules
    ...
    # Features: compute desde settings + template_config
    ...
    sector = {
        ...
        "features": sector_features,  # Features reales desde BD, no hardcoded
    }
