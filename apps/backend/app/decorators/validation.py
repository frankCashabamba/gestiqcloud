...
def validate_uuid(value: str, field_name: str = "ID") -> UUID:
    ...
    try:
        ...
        raise HTTPException(status_code=400, detail=f"Invalid {field_name.lower()}: {value}")
...
def validate_resource_exists(
    get_resource_func: Callable[[Session, str, str], Any], resource_name: str = "Resource"
):
    """
...
def tenant_required(func: Callable) -> Callable:
    ...
    def wrapper(*args, **kwargs) -> Any:
        ...
        if not request:
            raise HTTPException(
                status_code=500, detail="Request object not found for tenant validation"
            )
