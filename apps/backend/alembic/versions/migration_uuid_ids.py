...
def upgrade() -> None:
    ...
    op.execute(
        "INSERT INTO core_rolempresa_new (tenant_id, nombre, descripcion, permisos, rol_base_id, creado_por_empresa) SELECT tenant_id, nombre, descripcion, permisos, NULL, creado_por_empresa FROM core_rolempresa"
    )
    ...
    op.execute(
        "ALTER TABLE core_perfilusuario ALTER COLUMN usuario_id TYPE UUID USING usuario_id::UUID"
    )
    op.execute(
        "ALTER TABLE core_perfilusuario ADD CONSTRAINT core_perfilusuario_usuario_id_fkey FOREIGN KEY (usuario_id) REFERENCES usuarios_usuarioempresa(id)"
    )
