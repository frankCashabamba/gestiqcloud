...
export async function procesarDocumento(file: File, authToken?: string): Promise<ProcesarDocumentoResult> {
    ...
    fd.append('file', file)
    const json = await apiFetch<any>('/api/v1/imports/process', {
        method: 'POST',
        body: fd,
        authToken,
    })
...
export async function guardarPendiente(payload: DatosImportadosCreate) {
    return apiFetch<DatosImportadosOut>('/api/v1/imports/save', { method: 'POST', body: JSON.stringify(payload) })
}

export async function listarPendientes() {
    return apiFetch<DatosImportadosOut[]>('/api/v1/imports/list')
}

export async function hayPendientes() {
    return apiFetch<{ hayPendientes: boolean }>('/api/v1/imports/has-pending')
}

export async function eliminarPendiente(id: number) {
    return apiFetch<{ ok: boolean }>(`/api/v1/imports/delete/${id}`, { method: 'DELETE' })
}

export async function enviarDocumento(id: number, authToken?: string) {
    return apiFetch(`/api/v1/imports/send/${id}`, { method: 'POST', authToken })
}

export async function actualizarPendiente(id: number, payload: Partial<DatosImportadosCreate>, authToken?: string) {
    return apiFetch<DatosImportadosOut>(`/api/v1/imports/update/${id}`, { method: 'PUT', body: JSON.stringify(payload), authToken })
}

export async function guardarBatch(payload: DatosImportadosCreate[], authToken?: string) {
    return apiFetch<DatosImportadosOut[]>(`/api/v1/imports/save-batch`, { method: 'POST', body: JSON.stringify(payload), authToken })
}

export async function enviarBatch(ids: number[], authToken?: string) {
    return apiFetch<any>(`/api/v1/imports/send-batch`, { method: 'POST', body: JSON.stringify(ids), authToken })
}
