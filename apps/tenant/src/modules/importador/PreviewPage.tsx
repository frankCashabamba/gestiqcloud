        ...
        >
                        ...
                        <div className="rounded-lg border border-slate-200 bg-white overflow-hidden">
                            ...
                            <div className="border-t border-slate-200 bg-slate-50 px-4 py-3 flex items-center justify-between">
                                ...
                                <button
                                    ...
                                    title={sinCategoria > 0 ? 'Asigna categorías a todos los productos antes de promover' : ''}
                                >
                                    {promoting ? 'Promocionando...' : `Promover a producción`}
                                </button>
                            </div>
                        </div>
                    )}
            ...
            {totalProductos > pageSize && (
                <div className="mt-4 flex items-center justify-between text-sm text-slate-700">
                    <div>
                        Mostrando {(page - 1) * pageSize + 1} - {Math.min(page * pageSize, totalProductos)} de {totalProductos}
                    </div>
                    <div className="flex items-center gap-2">
                        <button
    ...
    return (
        <>
            {!open ? (
                ...
                >Reasignar mapping + Reprocesar</button>
            ) : (
                <div className="flex items-center gap-2">
                    <select className="rounded-md border border-slate-200 px-2 py-1 text-sm" value={selected} onChange={(e) => setSelected(e.target.value)}>
                        <option value="">Selecciona un mapping…</option>
                        {mappings.map((m) => (<option key={m.id} value={m.id}>{m.name}</option>))}
                    </select>
                    <button className="rounded-md bg-blue-600 px-3 py-2 text-sm font-medium text-white hover:bg-blue-500 disabled:opacity-50" onClick={apply} disabled={!selected || loading}>{loading ? 'Aplicando…' : 'Aplicar y reprocesar'}</button>
                    <button className="rounded-md border border-slate-200 px-3 py-2 text-sm text-slate-700 hover:bg-slate-100" onClick={() => setOpen(false)}>Cancelar</button>
                </div>
            )}
        </>
    )
