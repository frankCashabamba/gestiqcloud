...
export default function StockList() {
  ...
  const getAlertaInfo = (item: StockItem) => {
    ...
    if (min && item.qty <= min) {
      return { tipo: 'bajo', texto: 'Low stock', color: 'bg-red-100 text-red-800' }
    }
    if (max && item.qty > max) {
      return { tipo: 'sobre', texto: 'Overstock', color: 'bg-orange-100 text-orange-800' }
    }
    return null
  }
      ...
      <div className="bg-white shadow-sm rounded-lg p-4 mb-6 space-y-4">
        ...
        <div className="flex items-center justify-between text-sm">
          ...
          <div className="text-gray-600">
            <span className="font-medium">{filtered.length}</span> products in stock
          </div>
        </div>
      </div>
            ...
            <table className="min-w-full divide-y divide-gray-200">
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                ...
                {!loading && items.length === 0 && (
                  <tr>
                    <td colSpan={8} className="px-4 py-12 text-center">
                      ...
                      <p className="text-gray-500 mb-2">No stock registered</p>
                      <button
                ...
                {!loading && items.length > 0 && view.length === 0 && (
                  <tr>
                    <td colSpan={8} className="px-4 py-12 text-center">
                      <p className="text-gray-500">No products found with those filters</p>
                    </td>
                  </tr>
                )}
              </tbody>
