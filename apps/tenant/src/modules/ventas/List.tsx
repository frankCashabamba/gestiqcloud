...
export default function VentasList() {
  ...
  function exportCSV(rows: Venta[]) {
    const header = ['id','fecha','cliente_id','total','estado']
    const body = rows.map(r => [r.id, r.fecha, r.cliente_id ?? '', r.total, r.estado ?? ''])
          ...
          {view.map((v) => (
            <tr key={v.id} className="border-b">
              ...
              <td>
                ...
                <button className="text-red-700" onClick={async () => { if (!confirm('¿Eliminar venta?')) return; try { await removeVenta(v.id); setItems((p)=>p.filter(x=>x.id!==v.id)); success('Venta eliminada') } catch(e:any){ toastError(getErrorMessage(e)) } }}>Eliminar</button>
              </td>
            </tr>
          ))}
