    ...
    return (
        <div className="p-4">
            <div className="flex justify-between items-center mb-3">
                ...
                <button className="bg-blue-600 text-white px-3 py-1 rounded" onClick={() => nav('new')}>{t('productions:new')}</button>
            </div>
            <input value={q} onChange={(e) => setQ(e.target.value)} placeholder={t('productions:searchPlaceholder')} className="mb-3 w-full px-3 py-2 border rounded text-sm" />
            ...
            <table className="min-w-full text-sm">
                <thead>
                    <tr className="text-left border-b">
                        <th><button className="underline" onClick={() => { setSortKey('numero'); setSortDir(d => d === 'asc' ? 'desc' : 'asc') }}>{t('productions:number')} {sortKey === 'numero' ? (sortDir === 'asc' ? '▲' : '▼') : ''}</button></th>
                        <th><button className="underline" onClick={() => { setSortKey('status'); setSortDir(d => d === 'asc' ? 'desc' : 'asc') }}>{t('productions:status')} {sortKey === 'status' ? (sortDir === 'asc' ? '▲' : '▼') : ''}</button></th>
                        <th><button className="underline" onClick={() => { setSortKey('scheduled_date'); setSortDir(d => d === 'asc' ? 'desc' : 'asc') }}>{t('productions:date')} {sortKey === 'scheduled_date' ? (sortDir === 'asc' ? '▲' : '▼') : ''}</button></th>
                ...
                <tbody>
                    {view.map((o) => (
                        <tr key={o.id} className="border-b">
                            ...
                            <td>
                                <Link to={`${o.id}/editar`} className="text-blue-600 hover:underline mr-3">{t('productions:edit')}</Link>
                    ...
                    {!loading && items.length === 0 && (<tr><td className="py-3 px-3" colSpan={6}>{t('productions:empty')}</td></tr>)}
                </tbody>
            </table>
