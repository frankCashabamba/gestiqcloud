import React from 'react'

export default function ReconciliationPlaceholder() {
  return (
    <div className="p-6">
      <div className="max-w-2xl mx-auto text-center">
        <div className="text-6xl mb-4">🔄</div>
        <h1 className="text-2xl font-bold text-gray-800 mb-2">
          Conciliación Bancaria
        </h1>
        <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4 mb-6">
          <span className="text-yellow-800 font-medium">🚧 Módulo en desarrollo</span>
        </div>
        <p className="text-gray-600 mb-4">
          Este módulo permitirá conciliar movimientos bancarios con registros
          contables y de caja para mantener la integridad financiera.
        </p>
        <div className="bg-gray-50 rounded-lg p-4 text-left">
          <h3 className="font-semibold text-gray-700 mb-2">Funcionalidades planificadas:</h3>
          <ul className="text-sm text-gray-600 space-y-1">
            <li>• Importación de extractos bancarios</li>
            <li>• Matching automático de transacciones</li>
            <li>• Resolución de diferencias</li>
            <li>• Reportes de conciliación</li>
            <li>• Historial de ajustes</li>
          </ul>
        </div>
        <div className="mt-6 text-xs text-gray-400">
          Backend: apps/backend/app/modules/reconciliation
        </div>
      </div>
    </div>
  )
}
