...
const createLine = (taxRate = 0): WorkshopLine => ({
  description: '',
  quantity: 1,
  unit_price: 0,
  tax_rate: taxRate,
...
export default function TallerFacturaPage() {
  ...
  const [lines, setLines] = useState<WorkshopLine[]>([createLine()])
  ...
  const update = (index: number, next: Partial<WorkshopLine>) =>
    ...
    try {
      ...
      const payloadLines = lines.map((line) => {
        ...
        return {
          ...
          spare_part: line.spare_part || line.description,
          labor_hours: line.labor_hours || 0,
          rate: line.rate || unitPrice,
        }
      })
      ...
      success(t('billing.sectorInvoice.createdWorkshop'))
      nav('/invoicing')
    } catch (e: any) {
          ...
          {lines.map((line, index) => (
            <div key={index} className="border rounded p-3">
              <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                ...
                <div>
                  <label className="block mb-1">{t('billing.sectorInvoice.fields.type')}</label>
                  <select className="border px-2 py-1 w-full rounded" value={line.line_type} onChange={(e) => update(index, { line_type: e.target.value as any })}>
                    <option value="part">{t('billing.sectorInvoice.fields.part')}</option>
                    <option value="labor">{t('billing.sectorInvoice.fields.labor')}</option>
                  </select>
                </div>
                {line.line_type === 'part' && (
                  <>
                    <div>
                      <label className="block mb-1">{t('billing.sectorInvoice.fields.sparePart')}</label>
                      <input className="border px-2 py-1 w-full rounded" value={line.spare_part || ''} onChange={(e) => update(index, { spare_part: e.target.value })} />
                    </div>
                    <div>
                      <label className="block mb-1">{t('billing.sectorInvoice.fields.licensePlate')}</label>
                      <input className="border px-2 py-1 w-full rounded" value={line.license_plate || ''} onChange={(e) => update(index, { license_plate: e.target.value })} />
                    </div>
                  </>
                )}
                {line.line_type === 'labor' && (
                  <>
                    <div>
                      <label className="block mb-1">{t('billing.sectorInvoice.fields.hours')}</label>
                      <input type="number" min={0} className="border px-2 py-1 w-full rounded" value={line.labor_hours || 0} onChange={(e) => update(index, { labor_hours: Number(e.target.value) })} />
                    </div>
                    <div>
                      <label className="block mb-1">{t('billing.sectorInvoice.fields.rate')}</label>
                      <input type="number" step="0.01" min={0} className="border px-2 py-1 w-full rounded" value={line.rate || 0} onChange={(e) => update(index, { rate: Number(e.target.value) })} />
                    </div>
                  </>
                )}
              </div>
