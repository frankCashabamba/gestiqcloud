          ...
          {lineas.map((l, i) => (
            <div key={i} className="border rounded p-3">
              <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                ...
                <div><label className="block mb-1">{t('billing.sectorInvoice.fields.type')}</label><select className="border px-2 py-1 w-full rounded" value={l.tipo} onChange={(e)=> update(i,{ tipo: e.target.value as any })}><option value="pieza">{t('billing.sectorInvoice.fields.part')}</option><option value="mano_obra">{t('billing.sectorInvoice.fields.labor')}</option></select></div>
                {l.tipo === 'pieza' && (
                  <>
                    <div><label className="block mb-1">{t('billing.sectorInvoice.fields.sparePart')}</label><input className="border px-2 py-1 w-full rounded" value={l.repuesto || ''} onChange={(e)=> update(i,{ repuesto: e.target.value })} /></div>
                    <div><label className="block mb-1">{t('billing.sectorInvoice.fields.licensePlate')}</label><input className="border px-2 py-1 w-full rounded" value={l.vehiculo_matricula || ''} onChange={(e)=> update(i,{ vehiculo_matricula: e.target.value })} /></div>
                  </>
                )}
                {l.tipo === 'mano_obra' && (
                  <>
                    <div><label className="block mb-1">{t('billing.sectorInvoice.fields.hours')}</label><input type="number" min={0} className="border px-2 py-1 w-full rounded" value={l.horas_mano_obra || 0} onChange={(e)=> update(i,{ horas_mano_obra: Number(e.target.value) })} /></div>
                    <div><label className="block mb-1">{t('billing.sectorInvoice.fields.rate')}</label><input type="number" step="0.01" min={0} className="border px-2 py-1 w-full rounded" value={l.tarifa || 0} onChange={(e)=> update(i,{ tarifa: Number(e.target.value) })} /></div>
                  </>
                )}
              </div>
