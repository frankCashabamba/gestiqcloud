...
export default function ProveedoresList() {
  ...
  useEffect(() => {
    (async () => {
      try { setLoading(true); setItems(await listProveedores()) }
          ...
          {view.map((c) => (
            <tr key={c.id} className="border-b">
              ...
              <td>
                ...
                <button className="text-red-700" onClick={async () => { if (!confirm('¿Eliminar proveedor?')) return; try { await removeProveedor(c.id); setItems((p)=>p.filter(x=>x.id!==c.id)); success('Proveedor eliminado') } catch(e:any){ toastError(getErrorMessage(e)) } }}>Eliminar</button>
              </td>
            </tr>
          ))}
