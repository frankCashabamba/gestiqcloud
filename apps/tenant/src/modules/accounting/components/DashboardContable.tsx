  ...
  return (
    <div style={{ display: 'grid', gap: 12 }}>
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: 12 }}>
        <Box label={t('accounting.dashboard.totalIncome')} value={`$ ${ingresos.toFixed(2)}`} />
        <Box label={t('accounting.dashboard.totalExpenses')} value={`$ ${gastos.toFixed(2)}`} />
        <Box label={t('accounting.dashboard.result')} value={`$ ${resultado.toFixed(2)}`} />
      </div>
    </div>
  )
}
