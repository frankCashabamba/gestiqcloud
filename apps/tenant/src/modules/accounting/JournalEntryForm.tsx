...
export default function AsientoForm() {
    ...
    const onSubmit: React.FormEventHandler = async (e) => {
        e.preventDefault()
        try {
            if (!form.numero || !form.fecha) throw new Error(t('accounting.journalEntryForm.errors.numberDateRequired'))
            if (!form.lineas || form.lineas.length === 0) throw new Error(t('accounting.journalEntryForm.errors.atLeastOneLine'))

            const { debe, haber, balance } = calcularTotales()
            if (Math.abs(balance) > 0.01) throw new Error(t('accounting.journalEntryForm.errors.unbalanced', { debit: debe.toFixed(2), credit: haber.toFixed(2) }))
            ...
            success(t('accounting.journalEntryForm.saved'))
            nav('..')
        } catch (e: any) {
            ...
            <form onSubmit={onSubmit} className="space-y-4">
                ...
                <div className="border-t pt-4">
                    <div className="flex justify-between items-center mb-3">
                        <h4 className="font-semibold">{t('accounting.journalEntryForm.linesTitle')}</h4>
                        <button
                            type="button"
                            onClick={addLinea}
                    ...
                    <table className="w-full border">
                        <thead className="bg-gray-100">
                            <tr>
                                ...
                                <th className="border px-2 py-1 text-right">{t('accounting.journalEntries.columns.credit')}</th>
                                <th className="border px-2 py-1 text-left">{t('common.description')}</th>
                            ...
                            <tr>
                                ...
                                <td className="border px-2 py-2" colSpan={2}>
                                    {Math.abs(totales.balance) < 0.01 ? (
                                        <span className="text-green-600">✓ {t('accounting.journalEntryForm.balanced')}</span>
                                    ) : (
                                        <span className="text-red-600">✗ {t('accounting.journalEntryForm.unbalanced', { balance: totales.balance.toFixed(2) })}</span>
                                    )}
                                </td>
                            </tr>
                        </tfoot>
