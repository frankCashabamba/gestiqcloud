    ...
    <div className={`border rounded-lg p-4 ${estadoColor}`}>
      <div className="flex justify-between items-start mb-2">
        <div>
          ...
          <p className="text-xs mt-1">Employee: {vacacion.empleado_id}</p>
        </div>
      ...
      <div className="text-sm space-y-1">
        <p>
          <strong>From:</strong> {vacacion.fecha_inicio}
        </p>
        <p>
          <strong>To:</strong> {vacacion.fecha_fin}
        </p>
        <p>
          <strong>Days:</strong> {vacacion.dias}
        </p>
        {vacacion.motivo && (
          <p>
            <strong>Reason:</strong> {vacacion.motivo}
          </p>
        )}
        {vacacion.notas && (
      ...
      {vacacion.aprobado_por && (
        <p className="text-xs text-gray-600 mt-3">
          {vacacion.estado === 'aprobada' ? 'Approved' : 'Rejected'} by: {vacacion.aprobado_por}
          {vacacion.fecha_aprobacion && ` on ${vacacion.fecha_aprobacion}`}
        </p>
      )}
    </div>
  )
