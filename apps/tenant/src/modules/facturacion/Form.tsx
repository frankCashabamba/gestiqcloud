...
export default function FacturaForm() {
  ...
  return (
    <div className="p-4">
      <h3 className="text-xl font-semibold mb-3">{id ? 'Editar factura' : 'Nueva factura'}</h3>
      <form onSubmit={onSubmit} className="space-y-4" style={{ maxWidth: 520 }}>
        ...
        <div><label className="block mb-1">Total</label><input type="number" step="0.01" value={form.total} onChange={(e)=> setForm({ ...form, total: Number(e.target.value) })} className="border px-2 py-1 w-full rounded" required /></div>
        <div><label className="block mb-1">Estado</label><select value={form.estado || 'borrador'} onChange={(e)=> setForm({ ...form, estado: e.target.value })} className="border px-2 py-1 w-full rounded"><option value="borrador">Borrador</option><option value="emitida">Emitida</option><option value="anulada">Anulada</option></select></div>
