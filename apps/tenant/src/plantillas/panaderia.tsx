...
const PanaderiaPlantilla: React.FC<{ slug?: string }> = ({ slug }) => {
  ...
  if (has('ventas')) { kpis.push(kpi('k1', 'Ventas hoy')); kpis.push(kpi('k2', 'Tickets')) }
  if (has('inventario')) { kpis.push(kpi('k3', 'Stock bajo')); kpis.push(kpi('k4', 'Mermas')) }

  return (
    <SectorLayout
      title="Sector Panadería"
    ...
    >
      <div style={{ border: '1px solid var(--color-border)', background: 'var(--color-surface)', borderRadius: 12, padding: 16 }}>
        Plantilla de sector para “{slug ?? 'panaderia'}”. Personaliza KPIs y navegación aquí.
      </div>
    </SectorLayout>
  )
}
